...
//  Only these emails can access the Ticket Approvals tab and
//  approve / reject operator resolutions.
// ==============================================================
const SUPERADMIN_EMAILS = ['anthony.co@jiangnanhotpot.com'];

function isSuperAdmin(user) {
    if (!user || !user.email) return false;
    return SUPERADMIN_EMAILS.map(e => e.toLowerCase()).includes(user.email.toLowerCase());
}

// Whether the currently logged-in user is a superadmin.
function currentUserIsSuperAdmin() {
    return isSuperAdmin(auth.currentUser);
}
...
function renderApprovalAttachments(ticket) {
    const grid = approvalAttachmentsGrid;
    if (!grid) return;
    const resolution = (ticket && ticket.resolution) || {};
    // Fall back across the nested `resolution.attachments`, top-level `attachments`,
    // and the single `resolutionAttachmentUrl` string written by older/other flows.
    ...
    grid.innerHTML = atts.map((att, index) => {
...
window.openApprovalDetails = function(id) {
    ...
    if (approvalDetailsBody) {
        approvalDetailsBody.innerHTML = `
            <div class="modal-field"><label>Ticket Number</label><span>${escapeHTML(ticket.ticketNumber || id)}</span></div>
            <div class="modal-field"><label>Branch</label><span>${escapeHTML(ticket.branch || '-')}</span></div>
            <div class="modal-field"><label>Status</label><span>${escapeHTML(getApprovalStatusText(ticket))}</span></div>
            ...
            <div class="modal-field"><label>Resolved By</label><span>${escapeHTML(resolvedBy)}</span></div>
            <div class="modal-field"><label>Resolved At</label><span>${escapeHTML(resolvedAt)}</span></div>
            <div class="modal-field full-width"><label>Description</label><span>${escapeHTML(ticket.description || 'No description.')}</span></div>
            <div class="modal-field full-width"><label>Resolution Notes</label><span>${escapeHTML(notes)}</span></div>
            ${rejection.reason ? `<div class="modal-field full-width"><label>Rejection Reason</label><span>${escapeHTML(rejectedReason)}</span></div>` : ''}
        `;
    }
...
window.openTicketModal = function(id) {
    ...
    ticketModalBody.innerHTML = `
        ...
        ${resolutionHTML}
    `;
// ===== Attachments shown ONLY for resolved / for-revision tickets =====
    const attachmentsSection = document.getElementById('ticketAttachmentsSection');
    if (attachmentsSection) {
        attachmentsSection.style.display = ticketCanShowAttachments(ticket) ? '' : 'none';
    }
...
function switchTab(tabId) {
    ...
    const titles = { dashboard: 'Dashboard', branches: 'Branch Monitor', history: 'Status History', tickets: 'Tickets', approvals: 'Ticket Approvals' };
    const subtitles = { dashboard: 'Overview & Analytics', branches: 'Real-time Branch Health Status', history: 'Status Change Logs', tickets: 'Incident Ticket Management', approvals: 'Superadmin Approval Workflow' };
