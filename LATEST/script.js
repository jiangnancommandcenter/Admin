...
function setAttachmentStatus(message, isError) {
    const el = document.getElementById('attachmentUploadStatus');
    if (!el) return;
    el.textContent = message || '';
    el.style.color = isError ? 'var(--color-danger)' : 'var(--text-secondary)';
}
        ...
        return `
            <div class="attachment-item" data-public-id="${escapeHTML(att.public_id || '')}">
                ...
                <button type="button" class="attachment-remove" data-tooltip="Remove"
                    onclick="removeTicketAttachment('${escapeHTML(att.public_id || '')}')">
...
async function cloudinaryUpload(file, ticketId) {
...
async function uploadTicketAttachment() {
    if (!currentTicketId) {
        setAttachmentStatus('No ticket selected.', true);
        return;
    }
    if (!isCloudinaryConfigured()) {
        setAttachmentStatus('⚠️ Cloudinary is not configured yet. Set CLOUDINARY_CLOUD_NAME and CLOUDINARY_UPLOAD_PRESET in script.js.', true);
        return;
    }

    const input = document.getElementById('ticketAttachmentInput');
    if (!input || !input.files || input.files.length === 0) {
        setAttachmentStatus('Choose a file first.', true);
        return;
    }
    ...
    for (const file of files) {
        ...
        if (!typeOk) {
            setAttachmentStatus(`"${file.name}" type is not allowed.`, true);
            return;
        }
        if (!sizeOk) {
            setAttachmentStatus(`"${file.name}" exceeds ${MAX_ATTACHMENT_SIZE_MB}MB.`, true);
            return;
        }
    }
    ...
    setAttachmentStatus('Uploading...', false);

    try {
        const uploaded = [];
        for (const file of files) {
            const att = await cloudinaryUpload(file, currentTicketId);
            uploaded.push(att);
        }

        // Merge with existing attachments in the ticket doc
        const ticketDoc = await db.collection('tickets').doc(currentTicketId).get();
        const existing = (ticketDoc.exists && Array.isArray(ticketDoc.data().attachments)) ? ticketDoc.data().attachments : [];
        await db.collection('tickets').doc(currentTicketId).update({
            attachments: existing.concat(uploaded)
        });

        // Refresh local ticket state + re-render
        const idx = allTickets.findIndex(t => t.id === currentTicketId);
        if (idx !== -1) {
            allTickets[idx].attachments = existing.concat(uploaded);
            renderTicketAttachments(allTickets[idx]);
        }

        input.value = '';
        setAttachmentStatus(`Uploaded ${uploaded.length} file(s).`, false);
        if (uploadBtn) uploadBtn.disabled = false;
        setTimeout(() => setAttachmentStatus(''), 4000);
    } catch (error) {
        console.error('Upload error:', error);
        setAttachmentStatus('Upload failed: ' + error.message, true);
        if (uploadBtn) uploadBtn.disabled = false;
    }
}
...
async function removeTicketAttachment(publicId) {
    ...
    if (!confirm('Remove this attachment from the ticket?')) return;

    try {
        const ticketDoc = await db.collection('tickets').doc(currentTicketId).get();
        if (!ticketDoc.exists) return;
        const existing = Array.isArray(ticketDoc.data().attachments) ? ticketDoc.data().attachments : [];
        const remaining = existing.filter(a => a.public_id !== publicId);
        await db.collection('tickets').doc(currentTicketId).update({ attachments: remaining });
